...
train_selector <- sample(1:nrow(slope), round(nrow(slope) * train_ratio))
...
cor(train$alt, predict(m1, newdata = train))^2
cor(train$alt, predict(m2, newdata = train))^2
cor(train$alt, predict(m3, newdata = train))^2

```

```{r, echo=F}

cor(test$alt, predict(m1, newdata = test))^2
cor(test$alt, predict(m2, newdata = test))^2
cor(test$alt, predict(m3, newdata = test))^2
...
train_selector <- sample(1:nrow(evidence), round(nrow(evidence) * train_ratio))
